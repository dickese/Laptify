...
public class PaymentController {
    ...
    /**
     * VNPay return URL (browser GET with vnp_* query params). Verifies the signature and reports
     * the outcome. A production setup should also register VNPay's server-to-server IPN URL.
     */
    @GetMapping("/vnpay/callback")
    public ResponseEntity<Map<String, Object>> vnpayCallback(@RequestParam Map<String, String> params) {
        PaymentCallbackResult result = paymentService.handleCallback(PaymentMethod.VNPAY, params);
    ...
    public ResponseEntity<Void> momoCallback(@RequestBody Map<String, Object> body) {
        paymentService.handleCallback(PaymentMethod.MOMO, stringify(body));
    ...
    public ResponseEntity<Map<String, Object>> zalopayCallback(@RequestBody Map<String, Object> body) {
        PaymentCallbackResult result = paymentService.handleCallback(PaymentMethod.ZALOPAY, stringify(body));
        return ResponseEntity.ok(Map.of(
